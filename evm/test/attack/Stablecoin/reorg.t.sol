...
contract StablecoinReorgTest is StablecoinTest {
    ...
    function test_attack_reorg_mintRateLimit_timestampRollbackCannotInflateCapacity(uint256 limit, uint256 interval)
        public
    {
    ...
    function test_attack_reorg_mintRateLimit_overflowProtection(uint256 limit, uint256 interval, uint256 elapsed)
        public
    {
        limit = bound(limit, 1, type(uint128).max);
        interval = bound(interval, 1, uint256(type(uint40).max));
